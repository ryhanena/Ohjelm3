...
Engineer::Engineer(const std::shared_ptr<Course::iGameEventHandler> &eventhandler,
...
bool Engineer::canBePlacedOnTile(const std::shared_ptr<Course::TileBase> &target) const
{
    return target->getOwner() == getOwner() and
            WorkerBase::canBePlacedOnTile(target);
}

void Engineer::doSpecialAction()
{

}
const Course::ResourceMapDouble Engineer::tileWorkAction()
{
